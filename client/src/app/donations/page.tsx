...
import CharitiesPage, { Charity } from "./charities/page";
...
export default function DonationListPage() {
  ...
  const [charities, setCharities] = useState<ListApiResponse<Charity>>({
    hasMore: false,
    total: 0,
    data: [],
  });
  const [page, setPage] = useState(1);
  // Distinguish between first render and search
    ...
    async (searchText?: string) => {
      try {
        ...
        if (activeTab === "charities") {
          setCharities(data as ListApiResponse<Charity>);
  ...
  const fetchMoreData = useCallback(async () => {
    ...
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_SERVER_HOST}/api/${activeTab}?page=${
        page + 1
      }&limit=10${searchText ? `&name=${searchText}` : ""}`,
      {
        method: "GET",
      }
    );
  ...
  // fetch data when search block is unvisible or activeTab changes
  useEffect(() => {
